...
def read_data(
    ...
    """
    pararel = pd.read_csv(f"{permanent_path}/{filename}")
    data = pararel
    if test_on_head:
...
def build_prompt_for_parametric_knowledge_generation(
    ...
    pre_prompt_for_parametric_knowledge_generation_gpt2: str = PRE_PROMPT_FOR_PARAMETRIC_KNOWLEDGE_GENERATION_GPT2,
) -> str:
    ...
    if model_name == "gpt2-small":
        generation_template = pre_prompt_for_parametric_knowledge_generation_gpt2
    else:
        generation_template = pre_prompt_for_parametric_knowledge_generation
    ...
    prompt = generation_template.replace("<OBJECT_GENERATION_DESCRIPTION>", object_generation_description)
    prompt = prompt.replace("<STATEMENT_QUERY>", f"{statement_subject.strip()} {statement_relation.strip()}")
...
def generate_parametric_knowledge_dataset(
    ...
    data["parametric_output"] = data.model_output
...
def generate_counter_parametric_knowledge_dataset(
    ...
    for _, param_knowledge_record in tqdm(parametric_knowledge.iterrows()):
        ...
        counter_parametric_knowledge_list += build_counter_parametric_knowledge(
            ...
            statement_relation=param_knowledge_record.statement_relation,
            ...
            parametric_output=param_knowledge_record.parametric_output,
        )
...
def detect_subject_object_overlap(subject_str: str, object_str: str, string_similarity_threshold: float) -> bool:
    ...
    Otherwise, return True if the **cased** normalized jaro-winkler similarity between the two
...
def return_entity_overlap_between_relation_groups(parametric_knowledge_dataset: pd.DataFrame) -> dict:
    ...
    Args:
        parametric_knowledge_dataset (dict): the parametric knowledge dataset
    """
